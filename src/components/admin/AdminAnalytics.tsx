...
import { Download, RefreshCcw, BarChart3, Users } from "lucide-react";
import {
  BarChart,
  Bar,
  XAxis,
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        ...
        <div className="rounded-xl border border-border/60 bg-secondary/40 p-3">
          ...
          <div className="mt-1 font-display text-2xl font-bold">
            {leads.filter((l) => Date.now() - new Date(l.created_at).getTime() < 86_400_000).length}
          </div>
        </div>
        <div className="rounded-xl border border-border/60 bg-secondary/40 p-3">
          <div className="text-xs text-muted-foreground">Last 7d</div>
          <div className="mt-1 font-display text-2xl font-bold">
            {leads.filter((l) => Date.now() - new Date(l.created_at).getTime() < 7 * 86_400_000).length}
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-border/60 p-4 space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <h3 className="font-display font-semibold flex items-center gap-2">
            <BarChart3 className="size-4 text-primary" /> Leads over time
          </h3>
        ...
        <div className="max-h-[40vh] overflow-auto rounded-lg border border-border/40">
          <table className="w-full text-xs">
            ...
            <tbody>
              ...
              {leads.length === 0 && (
                <tr><td colSpan={4} className="px-2 py-6 text-center text-muted-foreground">{loading ? "Loading…" : "No leads yet"}</td></tr>
              )}
            </tbody>
          </table>
        </div>
